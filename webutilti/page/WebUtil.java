...
public class WebUtil {
	...
	public void implicitWait(int enterWaitTimeInSecond) {
		try {
			driver.manage().timeouts().implicitlyWait(java.time.Duration.ofSeconds(enterWaitTimeInSecond));
		} catch (Exception e) {
	...
	public void type(WebElement we, String sendValue, String elementName) {
		try {
			...
			try {
				...
				et.log(Status.INFO, "Data entered successfuly throug javaScript in " + elementName + " here Data is - "
						+ sendValue);
	...
	public void switchToWindow_WithoutcloseRemainingTab(String titleOrURL_full) {
		try {
			// Setwhs = driver.getWindowHandles();
	...
	public void takeScreenShot(String elementName) {
		try {
			...
			File locationOFSS = new File(screenshotPath);
	...
	public void clear(WebElement we, String name) {
		try {
			we.clear();
			et.log(Status.INFO, name + " Successfulliy clear value");
		} catch (Exception e) {

			et.log(Status.FAIL, " Anwanted    exception  please check  ");
	...
	public void verifyAttributeValue(WebElement we, String attributeName, String elementName,
	...
	public void waitForElementVisibility(WebElement we, int maxTimeout) {
		...
		WebDriverWait wt = new WebDriverWait(driver, java.time.Duration.ofSeconds(maxTimeout));
	...
	public void waitForElementInvisibility(WebElement we, int maxTimeout) {
		...
		// WebDriverWait wt = new WebDriverWait(driver,
		// Duration.microseconds(maxTimeout));
	...
	public void fluentWait(int timeout) {
		...
		flt.pollingEvery(java.time.Duration.ofSeconds(100));
		flt.withTimeout(java.time.Duration.ofSeconds(timeout));
		flt.ignoring(NoSuchElementException.class);

	}

	// """""""""""""""""' for the windowmaximize """""""""//
